#!/usr/bin/env python3
"""Stage 'build' para firmware_test_ota/ — variante OTA minimal.

Este firmware NO corre cámara ni inference; sólo WiFi + HTTP + OTA. Sirve
para aislar el pipeline OTA del stack de esp-dl/camera. Si acá el OTA anda
y en firmware_ota/ no, el problema no está en el OTA en sí.

Modos:
  default              build + HTTP POST /ota (requiere --ip)
  --initial            build + USB flash (primer flasheo, BOOT button)
  --build-only         sólo compila
  --info-only          GET /info al device (no compila)
  --menuconfig-only    idf.py menuconfig sobre firmware_test_ota/
"""
...
FIRMWARE_DIR_NAME = "firmware_test_ota"
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="script04c-build_test_ota.py", description=__doc__)
    p.add_argument(
        "--ip",
        default=None,
    ...
    p.add_argument(
        "--initial",
        action="store_true",
        help="USB flash (primera vez). Requiere BOOT button.",
    )
    p.add_argument(
        "--info-only",
        action="store_true",
        help="GET /info al device y salir (no compila).",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Preflight: sólo IDF env. No chequeamos model_data.h ni metadata
    # coherence — este firmware no los usa. Sí chequeamos PSRAM? No,
    # este build apaga PSRAM explícitamente.
    try:
        _s04.check_idf_env()
    except PreflightError as e:
        print(f"preflight error: {e}", file=sys.stderr)
