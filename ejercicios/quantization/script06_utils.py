...
_FMNIST_NPZ = _DATASETS_ROOT / "fashion_mnist.npz"

def load_and_preprocess_data():
    """
    Carga el dataset Fashion MNIST y lo escala a [0, 1].
    Descarga una vez en examples/datasets/fashion_mnist.npz y reutiliza.
    Retorna arrays con forma (batch_size, 28, 28, 1).
    """
    print("\n[INFO] Cargando dataset Fashion MNIST...")
    if _FMNIST_NPZ.exists():
        data = np.load(_FMNIST_NPZ)
        X_train, y_train = data["x_train"], data["y_train"]
        X_test, y_test   = data["x_test"],  data["y_test"]
        print(f"  → cargado desde caché: {_FMNIST_NPZ}")
    else:
        from tensorflow.keras.datasets import fashion_mnist
        (X_train, y_train), (X_test, y_test) = fashion_mnist.load_data()
        np.savez(_FMNIST_NPZ, x_train=X_train, y_train=y_train,
                               x_test=X_test,   y_test=y_test)
        print(f"  → guardado en caché: {_FMNIST_NPZ}")

    X_train_norm = X_train.astype('float32') / 255.0
    X_test_norm  = X_test.astype('float32')  / 255.0

    # Canal=1 para compatibilidad con Conv2D; Flatten resuelve (28,28,1)→784 igual.
...
# ---------------------------------------------------------------------------
# Constructores de configuración de cuantización (3 pasos)
# ---------------------------------------------------------------------------

def build_uniform_config(model, bits, layer_metrics, global_metrics):
    """
    Paso 1 — Cuantización Uniforme.
    Mismo número total de bits Y mismos bits enteros globalmente para todas las capas.
    Los bits enteros se derivan del máximo global de pesos/activaciones (peor caso global).
    """
    w_int      = get_integer_bits(global_metrics['w']['max'])
    b_int      = get_integer_bits(global_metrics['b']['max'])
    act_int    = get_integer_bits(global_metrics['act']['max'])
...
def build_mixed_intbits_config(model, bits, layer_metrics, clip_metric):
    """
    Paso 2 — Precisión Mixta en Bits Enteros.
    Mismo número total de bits para todas las capas, pero los bits enteros son
    per-capa, derivados de las estadísticas de cada capa y la métrica de clipping.
    clip_metric ∈ {'max', 'p95', 'p75'}
    """
    config = {}
    for layer in model.layers:
        if not layer.get_weights():
            continue
        w_val   = layer_metrics[layer.name]['w'][clip_metric]
        b_val   = layer_metrics[layer.name].get('b', {}).get(clip_metric, 0)
        act_val = layer_metrics.get(layer.name, {}).get('act', {}).get(clip_metric, 0)

        w_int        = get_integer_bits(w_val)
        b_int        = get_integer_bits(b_val)
        act_int_safe = max(0, get_integer_bits(act_val))
...
def build_full_mixed_config(model, layer_bits_map, layer_metrics, clip_metric):
    """
    Paso 3 — Precisión Mixta Completa.
    Diferente número total de bits por capa (definido en layer_bits_map) con bits enteros
    per-capa derivados de las estadísticas.  Permite asignar más bits a las capas
    primera y última (más sensibles) y menos a las capas interiores.
    clip_metric ∈ {'max', 'p95', 'p75'}
    """
    config = {}
    for layer in model.layers:
        if not layer.get_weights():
            continue
        bits    = layer_bits_map.get(layer.name, 8)
        w_val   = layer_metrics[layer.name]['w'][clip_metric]
        b_val   = layer_metrics[layer.name].get('b', {}).get(clip_metric, 0)
        act_val = layer_metrics.get(layer.name, {}).get('act', {}).get(clip_metric, 0)
...
def make_default_full_mixed_bits_map(model, high_bits=8, low_bits=4):
    """
    Asigna bits por capa: primeras y últimas capas entrenables → high_bits,
    capas intermedias → low_bits.
    Retorna dict {layer_name: n_bits}.
    """
    trainable_layers = [l for l in model.layers if l.get_weights()]
    bits_map = {}
    for i, layer in enumerate(trainable_layers):
        if i == 0 or i == len(trainable_layers) - 1:
            bits_map[layer.name] = high_bits
        else:
            bits_map[layer.name] = low_bits
    return bits_map


def plot_clipping_calibration(model, layer_name, save_path, bits=8):
    """
    Genera un gráfico de dos paneles para una capa dada:
      Izquierda: rango completo [-max, +max] con grilla de cuantización superpuesta.
      Derecha:   rango recortado al percentil p95 con grilla más fina (mejor resolución).

    Ilustra visualmente por qué recortar el rango mejora la precisión de cuantización
    en regiones de alta densidad del histograma.
    """
    ...
    w_max = float(max(abs(weights.min()), abs(weights.max())))
    p5, p95 = np.percentile(weights, [5, 95])
    w_p95   = float(max(abs(p5), abs(p95)))

    fig, axes = plt.subplots(1, 2, figsize=(11, 4))

    for ax, clip_range, title, color in [
        (axes[0], w_max,  f"Rango máximo  ±{w_max:.3f}  ({2**bits} niveles)", "#4C72B0"),
        (axes[1], w_p95,  f"Recortado p95 ±{w_p95:.3f}  ({2**bits} niveles)", "#55A868"),
    ]:
        ax.hist(weights, bins=80, alpha=0.55, color=color, edgecolor="none")
        n_levels = 2 ** bits
        step = (2 * clip_range) / (n_levels - 1)
        grid_vals = np.linspace(-clip_range, clip_range, n_levels)
        y_lims = ax.get_ylim()
        for gv in grid_vals:
            ax.axvline(gv, color="red", alpha=0.25, linewidth=0.6)
        ax.set_ylim(y_lims)
        ax.axvline(-clip_range, color="red", linewidth=1.2, linestyle="--", alpha=0.8)
        ax.axvline(+clip_range, color="red", linewidth=1.2, linestyle="--", alpha=0.8)
    ...
    fig.suptitle(
        f"Calibración de Clipping — capa '{layer_name}' ({bits} bits)\n"
        "Izquierda: pasos anchos → poca resolución en el centro.  "
        "Derecha: pasos finos → mejor resolución en el centro.",
        fontsize=9,
    )
    plt.tight_layout()
    plt.savefig(save_path, dpi=120, bbox_inches="tight")
    plt.close()
    print(f"[INFO] Gráfico de calibración guardado en '{save_path}'")
